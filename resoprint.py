...
class Screen():
            ...
            for x in range(columns):
                try:
                    ...
                    if self.maincolor == "random":
                        new_cell = [self.mainsymbol, self.colors[self.randomColor()]]
                    else:
                        new_cell = [self.mainsymbol, self.colors[self.maincolor]]
                row_list.append(new_cell)
            screen.append(row_list)
        self.screen = screen
        self.rows = rows
    ...
    def resetScreen(self, main_symbol = "--", main_color = "--"):
        ...
        for y in range(self.rows):
            for x in range(self.columns):
                if len(main_symbol)>1:
                    self.screen[y][x][0] = random.choice(main_symbol)
                else:
    ...
    def drawPyramid(self, row, column, size, symbol = "*", color = "red"):
        ...
        for i in range(size):
            for a in range(-count, count):
                ...
                if color == "random":
                    self.screen[row + d][column + a][1] = self.colors[self.randomColor()]
                else:
    ...
    def drawLine(self, x1, y1, x2, y2, symbol = "#", color = "red"):
        ...
        for x in range(distX + 1):
            ...
            if color == "random":
                self.screen[y1+math.floor(x * rpc * posY)][x1 + posX * x][1] = self.colors[self.randomColor()]
            else:
                self.screen[y1 + math.floor(x * rpc * posY)][x1 + posX * x][1] = self.colors[color]
    ...
    def drawHorizontalLine(self, x, y, length, symbol = "#", color = "red"):
        ...
        for i in range(abs(length)):
            ...
            if color == "random":
                self.screen[y][x + i * position][1] = self.colors[self.randomColor()]
            else:
    ...
    def drawNotepad(self, row = 1, column = 1, width = 50, height = 20, text = "", title = "", border_symbol = "█", color = "green", auto_background = False):
        ...
        notepad_text += border_symbol * \
            math.floor(width / 8) + title + border_symbol * (width - math.floor(width / 8) - len(title)) + "\n"
        notepad_text += border_symbol*width + "\n"
        linetext = ""
        linecount = 4
        for s in range(len(text)):
            ...
            if text[s] == "\n":
                linecount += 1
                notepad_text += border_symbol + linetext + (width - len(linetext) - 2) * " " + border_symbol + "\n"
                linetext = ""
                continue
            if text[s] == "\t":
                continue
    ...
    def getSymbolNeighbours(self, row, column):
        ...
        def oneNeighbour(newR, newC, rows = self.rows, columns = self.columns):
            if newR >= 0 and newR < self.rows and newC >= 0 and newC < self.columns:
                newN = [newR + 1, newC + 1, self.screen[newR][newC][0], findInDict(self.screen[newR][newC][1])]
                return newN
        row -= 1
        column -= 1
        returnList = []
    ...
    def findSymbol(self, symbol):
        ...
        for y in range(self.rows):
            for x in range(self.columns):
                if self.screen[y][x][0] == symbol:
                    symbols.append([y + 1, x + 1, self.screen[y][x][0], self.screen[y][x][1]])
        return symbols
